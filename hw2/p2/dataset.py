...
class CIFAR10Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ########################################################
        # 1. use Image.open() to load image according to the self.image_names  
        image = Image.open(os.path.join(self.dataset_dir, self.image_names[index]))
        # 2. apply transform on image
        image = self.transform(image)
        # 3. if not test set, return image and label with type "long tensor"   
        if(self.split != 'test'):
